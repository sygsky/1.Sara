...
_camp_hint = {
	...
	{
		_str = composeText[ format[ localize "STR_CAMP_19", _label   ], lineBreak, lineBreak, parseText("<t align='center'><t color='#ffffff00'>" + (localize "STR_CAMP_2")+ "</t></t>")];
...
if (isNil _visited) then
{
	_visited = false;
	(_this select 0) setVariable ["camp_is_visited",false];
};
if ( !_visited ) then
{
	...
	hint  localize format[ "campcheck.sqf: %1 score added to player for visitong camp #%2", d_ranked_a select INDEX_FOR_VISIT_SCORE , (_this select 3) select 0];
}
else
{
	hint  localize format[ "campcheck.sqf: camp %1 is already visited", (_this select 3) select 0];
}};
#endif
