/*
	...
	Main goal is to check if vehicle is overturned or not. If yes, tries to turn it on again.
	Is better than developed long time before, because on works really!
...
SYG_getOutEvent =
{
    ...
    hint localize "[[[                                                       ]]]";
    #endif
    hint localize format["[[[ * SYG_getOutEvent: ""%1"" event id %2, crew detected %3, vehicle tlist %4,  params %5 * ]]]",
                         EVENT_ID_VAR_NAME, _GetOutEventInd, count _whole_crew, _tlist, _this ];
    ...
    if ( !alive _veh ) exitWith // vehicle is dead, nothing to do with it
    {
        ...
        #ifdef __DEBUG_PRINT__
        hint localize format["--- SYG_getOutEvent: REMOVE EVENT vehicle %1 is dead, exit, time %2", _veh, time];
        #endif
        false
    };

    #ifdef __DEBUG_PRINT__
    if (damage _veh != 0 ) then {hint localize format["+++ SYG_getOutEvent: %1 has damage %2", _veh, damage _veh]};
    #endif
    ...
    {
        #ifdef __DEBUG_PRINT__
        hint localize format["<<< SYG_getOutEvent: %1 vehicle is down, exit >>>", _veh];
        #endif
    ...
    if ( !_udState && vehicle _first_man_out == _veh ) exitWith
    {
        ...
        hint localize format["<<< SYG_getOutEvent: jumped out man already moved inside %1, repair vehicle damage and exit >>>", _veh];
        true
    };
    ...
    if (!_udState) exitWith // Vehicle stands on feet, exit mow
    {
        // not overturned, exit
        _veh setVariable [EVENT_ID_VAR_NAME, _GetOutEventInd]; // restore event handling
        hint localize format["<<< SYG_getOutEvent: vehicle %1 not overturned, leave it as is, crew %1 >>>", _veh, count crew _veh];
        true
    };
...
//            };
    ...
    hint localize format["<<< SYG_getOutEvent: end of script for %1, roles assigned %2, crew %3 (alive %4), time %5", _veh, _tlist, count crew _veh, {alive _x} count crew _veh, time];
    true
};