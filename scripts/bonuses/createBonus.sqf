/*
    ...
    ...
	author: Sygsky
	description: place next bonus near the designated position according mode
	returns: nothing
...
//    player addAction ["Create SHIP","scripts\bonuses\createBonus.sqf", "SHIP"];
	...
	_str = "+++ bonus test module initiated";
    hint localize _str;
...
sleep 0.01;
_pos = [ _town_descr select 0, _town_rad * 1.5, _town_rad * 2 ] call XfGetRanPointAnnulusBig; // position for the bonus vehicle

switch ( toUpper _cmd ) do {
    case "AIR": {
        _id = mt_bonus_vehicle_array_AIR_arr call  SYG_findTargetBonusIndex;
        hint localize format["+++ AIR: id = %1", _id];
        _pos_arr = (_town_descr select 0) call _find_air_pos;
        player groupChat format["%1", _pos_arr];
        sleep 0.01;
        _dir = _pos_arr select 1;
        _pos = _pos_arr select 0;
    };
    case "TANK": {
...
if ( _cmd in ["TELE","BUMP_TARGET"]) exitWith{};

_veh_type = mt_bonus_vehicle_array_COM select _id;
_veh = _veh_type createVehicle  [0,0,0]; // createVehicle [ _veh_type, _pos, [], 0, "NONE" ];
_veh setDir _dir;
_veh setPos _pos;
_veh setFuel 0;
