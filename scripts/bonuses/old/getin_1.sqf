/*
    scripts\bonuses\getin_1.sqf : initial getin event for light version of bunus vehicle creation on island territory,
    							  not the base only
    author: Sygsky
	...
	After it getin event reassigned to getin_2.sqf file
	...
	called only one time when first player/AI enters this vehicle. After run removes itself and replace with "getin_2.sqf" script
                                    0               1               2               3               4
    "bonus" variable format: [_marker_name, _marker_type]
...
_setMarkerName = {
	...
	_bonus = [ _marker_name, _marker_type ];
...
//+++++++++++++++++
_veh = _this select 0;
if ( isNull _veh ) exitWith { hint localize "--- getin_1.sqf: isNull vehicle detected, exit script"; }; // that is an empty call

_unit = _this select 2;
if ( isNull _unit ) exitWith {hint localize "--- getin_1.sqf: isNull unit detected, exit script";};

// find player who entered vehicle
// TODO: detect if AI belongs to the some player
_player = "";
if( !isPlayer _unit ) exitWith {  hint localize format[ "--- getin_1.sqf: not player entered into %1, exit script", typeOf _veh ] };
