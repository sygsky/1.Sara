...
//		player groupChat "flaresoverbase.sqf: player entered";
	...
	if ( count _wounded > 0 ) then
	{
		for "_i" from 0 to count _wounded - 1 do
		{
			...
			if ( !isNull _unit AND alive _unit ) then
			{
				...
				{
#ifdef __DEBUG__
		player groupChat "flaresoverbase.sqf: wounded in in conscious";
		hint localize "flaresoverbase.sqf: wounded in in conscious";
#endif			
	...
	if ( _flare_launched < MAX_FLARE_NUMBER ) then  // few flares launched, check for more unconscious
	{
			...
			for "_i" from 0 to count _on_base_groups - 1 do
			{
				...
				else
				{
					{ // forEach units _grp;
						...
						if ( _unc ) exitWith // wounded man found man, add it to list ans exit
						{
							// launch flare now
		...
		if ( !_wounded_found ) then
		{
			player groupChat "flaresoverbase.sqf: no wounded found";
			hint localize    "flaresoverbase.sqf: no wounded found";
		};
#endif			
	};
	
	if ( (_flare_launched == 0 ) AND ( count _on_base_groups == 0 ) ) then 
	{
#ifdef __DEBUG__
			if ( !_wounded_found ) then
			{
